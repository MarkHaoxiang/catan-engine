...
- `rows.py` — the flat action table decoded once (device `ROW_TYPE` /
  ...
  here — per-module copies of the decode can silently diverge.
...
**Opt-in search flags, each awaiting a gated arena A/B under a learned value.**
Under the stationary heuristic leaf neither flag (nor more sims — 64 *loses*)
converted into strength, and the deleted engines they subsume (frozen-world
`mcts`, explicit-chance `smcts`) measured parity at best; the bet is that a
*learned* value (settlrl-learn q-blend, exp 0004) converts what the stationary
leaf couldn't.
- `chance_nodes` / `dev_chance` (`make_tree` /
  `make_search[_weights[_value]]`): the descent is a decision/chance state
  ...
  bit-identical; 13 baseline + 4 chance contracts).
- `ordered` (action-ordering lock-out): the descent
...
The ply-2 bias these fix: at ~32 sims trees are ~2 plies deep, so full-depth
selection over noisy follow-ups is the optimizer's curse — the measured failure
mode was END_TURN flipped to BUY_DEV/TRADE, losing 1-ply value. Defaults are
the local optimum (sims 64 loses, considered peaks at 16, prior_scale 5 loses,
...
~additive; experiment 0004). Kept for >= 3 players /
